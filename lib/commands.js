...
export function openTicket(commandData) {
  ...
  if (space > 0) {
    var slackCustomerUser = commandData.text.substring(0, space);
  } else {
  ...
  var subject = commandData.text.replace(slackCustomerUser, '').trim();
  ...
  .then(ticket => {
    ...
    var text = '<' + slackCustomerUser + '> A support ticket has been opened for your request. We contact you through the email address associated with your Slack account as soon as possible. <https://auth0.zendesk.com/agent/tickets/' + ticket.ticket.id + '|' + '[Open Ticket]>';
    var iconUrl = 'https://www.gravatar.com/avatar/7db6b16c9871854df6e522209e0a0631';
    return postSlackMessage(commandData.channel_id, text, 'support', iconUrl);
  });
}
